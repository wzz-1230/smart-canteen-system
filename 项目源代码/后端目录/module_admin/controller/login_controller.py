...
async def login(
    ...
    # 安全地操作 Redis，Redis 不可用时不影响登录流程
    async def _safe_redis_get(key):
        try:
    ...
    async def _safe_redis_set(key, value, ex=None):
        try:
            redis = getattr(request.app.state, 'redis', None)
            if redis is None:
                return
    ...
    captcha_enabled = (
        await _safe_redis_get(f'{RedisInitKeyConfig.SYS_CONFIG.key}:sys.account.captchaEnabled') == 'true'
    )
    user = UserLogin(
...
async def logout(request: Request, token: Annotated[str | None, Depends(oauth2_scheme)]) -> Response:
    try:
        # 防御性编程：token 可能为 None 或无效
        if token is None:
            logger.warning('logout: token 为空，直接返回退出成功')
        ...
        if AppConfig.app_same_time_login:
            token_id: str | None = payload.get('session_id')
        else:
            token_id: str | None = payload.get('user_id')
        if token_id is None:
            logger.warning('logout: token 中未找到有效 id，直接返回退出成功')
            return ResponseUtil.success(msg='退出成功')
        await LoginService.logout_services(request, str(token_id))
        logger.info('退出成功')
        return ResponseUtil.success(msg='退出成功')
    except Exception as e:
        logger.warning(f'logout 过程中出现异常: {e}，仍返回退出成功')
        return ResponseUtil.success(msg='退出成功')
