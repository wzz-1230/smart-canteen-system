...
service.interceptors.request.use(async config => {
  ...
  // 防御性处理：确保 token 为有效的非空字符串，避免将 'null'/'undefined' 注入到 header
  const tokenValue = getToken()
  if (tokenValue && typeof tokenValue === 'string' && tokenValue.trim() !== '' && !isToken) {
    config.headers['Authorization'] = 'Bearer ' + tokenValue // 让每个请求携带自定义token 请根据实际情况自行修改
  }
